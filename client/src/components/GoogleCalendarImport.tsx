...
import { Upload, Loader2, CheckCircle2, FileUp, Link2 } from "lucide-react";
...
function parseICalDateTime(value: string, tzid?: string): { dateTime: string; isAllDay: boolean; timezone?: string } {
  ...
  if (/^\d{8}$/.test(cleaned)) {
    ...
    return { dateTime: `${year}-${month}-${day}T09:00:00.000Z`, isAllDay: true, timezone: tzid };
  }
  ...
  if (/^\d{8}T\d{6}Z$/.test(cleaned)) {
    ...
    return { dateTime: `${year}-${month}-${day}T${hour}:${minute}:${second}.000Z`, isAllDay: false, timezone: tzid };
  }
  ...
  if (/^\d{8}T\d{6}$/.test(cleaned)) {
    ...
    const second = cleaned.substring(13, 15);
    if (tzid) {
      return { dateTime: localToUTC(year, month, day, hour, minute, second, tzid), isAllDay: false, timezone: tzid };
    }
...
function parseICalFile(content: string): ParsedICalEvent[] {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "END:VEVENT") {
      ...
      if (!startParsed.dateTime) { eventIndex++; continue; }

      let endParsed = endProp ? parseICalDateTime(endProp, endTzid) : null;
      ...
      if (!endParsed || !endParsed.dateTime) {
        if (startParsed.isAllDay) {
          endParsed = { dateTime: startParsed.dateTime.replace("T09:00:00", "T17:00:00"), isAllDay: true, timezone: startParsed.timezone };
        } else {
          const endDate = new Date(startParsed.dateTime);
          endDate.setHours(endDate.getHours() + 1);
          endParsed = { dateTime: endDate.toISOString(), isAllDay: false, timezone: startParsed.timezone };
        }
      }
    ...
    if (inEvent) {
      ...
      if (key.includes(";")) {
        ...
        for (const param of parts.slice(1)) {
          if (param.startsWith("TZID=")) { tzid = param.substring(5); }
        }
      }
...
function formatEventTime(dateTime: string, isAllDay: boolean): string {
  ...
  if (isAllDay) {
    return d.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }) + " (All day)";
  }
...
export function GoogleCalendarImport() {
  ...
  const processICalText = (text: string, source: string) => {
    if (!text || typeof text !== "string") {
      toast({ title: "Failed to parse calendar", description: "No calendar data was received.", variant: "destructive" });
      return;
    }
    try {
      const events = parseICalFile(text);
      if (events.length === 0) {
        toast({ title: "No events found", description: "The calendar data doesn't contain any events.", variant: "destructive" });
        return;
      }
      ...
      setFileName(source);
      toast({ title: `Found ${events.length} event${events.length !== 1 ? "s" : ""}`, description: "Select which events you'd like to import." });
    } catch (err) {
      console.error("[iCal parse error]", err);
      toast({ title: "Failed to parse calendar", description: "The data doesn't appear to be a valid iCal format.", variant: "destructive" });
    }
  };
  ...
  const fetchFromUrl = useMutation({
    ...
    onError: (err: any) => {
      const msg = err.message || "Could not fetch the calendar";
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!name.endsWith(".ics") && !name.endsWith(".ical")) {
      toast({ title: "Invalid file", description: "Please upload an .ics file.", variant: "destructive" });
      return;
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { setIsOpen(open); if (!open) resetState(); }}>
      <DialogTrigger asChild>
        <Button variant="outline" size="icon" title="Import Google Calendar" data-testid="button-import-gcal">
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>
            {step === "upload" ? "Import Google Calendar" : "Select Events to Import"}
          </DialogTitle>
          <DialogDescription>
            {step === "upload"
              ? "Paste a public Google Calendar link or upload an .ics file to import events."
          ...
          <div className="space-y-4 py-4">
            ...
            {inputMode === "url" && (
              <div className="space-y-3">
                <div className="space-y-2">
                  <Input
                    placeholder="Paste your Google Calendar public link..."
                    value={calendarUrl}
                    onChange={(e) => setCalendarUrl(e.target.value)}
                    onKeyDown={(e) => { if (e.key === "Enter" && calendarUrl.trim()) fetchFromUrl.mutate(); }}
                  />
                  <Button
                    className="w-full"
                  ...
                  >
                    {fetchFromUrl.isPending ? (
                      <><Loader2 className="w-4 h-4 animate-spin mr-2" />Fetching events...</>
                    ) : (
                      "Fetch Events"
                    )}
                  </Button>
            ...
            {inputMode === "file" && (
              <div className="space-y-3">
                ...
                <input ref={fileInputRef} type="file" accept=".ics,.ical" onChange={handleFileSelect} className="hidden" />
              </div>
            )}
          </div>
        )}
          ...
          <div className="flex flex-col flex-1 min-h-0 space-y-3 py-2">
            ...
            <div className="overflow-y-auto flex-1 border rounded-md divide-y max-h-[50vh]">
              {parsedEvents.map((event) => (
                <label key={event.id} className="flex items-start gap-3 p-3 hover:bg-muted/50 cursor-pointer">
                  <Checkbox checked={selectedEventIds.has(event.id)} onCheckedChange={() => toggleEvent(event.id)} className="mt-1" />
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm truncate">{event.title}</div>
                    <div className="text-xs text-muted-foreground mt-0.5">{formatEventTime(event.startTime, event.isAllDay)}</div>
                    {event.location && (
                      <div className="text-xs text-muted-foreground/70 mt-0.5 truncate">{event.location}</div>
                    )}
                    <div className="flex gap-1 mt-1">
                      {event.isAllDay && <Badge variant="outline" className="text-xs">All Day</Badge>}
                      {event.rrule && <Badge variant="secondary" className="text-xs">Recurring</Badge>}
                    </div>
                  </div>
                </label>
              ))}
          ...
          <DialogFooter>
            ...
            >
              {importEvents.isPending ? (
                ...
                <><CheckCircle2 className="w-4 h-4 mr-2" />Import {selectedEventIds.size} Event{selectedEventIds.size !== 1 ? "s" : ""}</>
              )}
            </Button>
          </DialogFooter>
        )}
