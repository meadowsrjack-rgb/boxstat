          ...
          export default function PlayerDashboard({ childId }: { childId?: number | null }) {
            ...
            const { data: awardsSummary, isLoading: awardsLoading } = useQuery({
            ...
            // ---- Real-time triggers (attendance/profile/modules) ----------------------
            useEffect(() => {
              ...
              socket.onmessage = (event) => {
                try {
                  ...
                  // Attendance trigger: coach scans player QR at check-in
                  if (data.type === "attendance_marked" && data.userId === currentUser.id) {
                    queryClient.invalidateQueries({ queryKey: ["/api/users", currentUser.id, "tasks"] });
                  }
                  // Profile bio trigger: profile saved & non-empty
            ...
            // ---- Helpers --------------------------------------------------------------
            const initials = `${(currentChild?.firstName || currentUser.firstName || "").charAt(0)}${(
            ...
            const upcomingEvents = useMemo(() => {
              ...
              return displayEvents
                ...
                .filter((ev) => {
                  // exclude today's items from "Upcoming"
                  return !isSameDay(new Date(ev.startTime || (ev as any).start_time), new Date());
                })
                .slice(0, 3);
            }, [displayEvents]);
                ...
                <header className="bg-white shadow-sm">
                  <div className="max-w-md mx-auto px-4 py-3">
                    <div className="flex items-center justify-between">
                      <Button variant="ghost" size="icon" onClick={() => setShowQR((s) => !s)} className="h-12 w-12">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <Button variant="ghost" size="icon" className="h-12 w-12" onClick={() => setLocation("/settings")}>
                  ...
                  <div className="px-6">
                    ...
                    {activeTab === "activity" && (
                      <div className="space-y-8">
                        ...
                        <section className="space-y-4">
                          ...
                          {/* Today tasks (not links; trigger-based) */}
                              ...
                              tasks
                                ...
                                .map((t) => (
                                  <div key={t.id} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
                                    ...
                                    {/* Only HOMEWORK is user-markable; others auto-complete via triggers */}
                                    {t.type === "HOMEWORK" ? (
                                      <Button
                                        size="sm"
                                ...
                                <div key={event.id} className="flex items-center justify-between p-3 bg-white rounded-lg shadow-sm">
                                  <div className="flex-1">
                                    ...
                                    <div className="flex items-center gap-4 text-xs text-gray-600 mt-1">
                                      <span className="flex items-center gap-1">
                                        <CalendarIcon className="w-3 h-3" />
                                        {format(new Date(event.startTime || (event as any).start_time), "MMM d")}
                                      </span>
                                      <span className="flex items-center gap-1">
                                        🕐 {format(new Date(event.startTime || (event as any).start_time), "h:mm a")}
                                      </span>
                        ...
                        {/* Note: Skills Ratings & Trophies/Badges sections REMOVED on first tab */}
                      </div>
                    )}
                    ...
                    {activeTab === "video" && (
                      <div className="flex flex-col items-center justify-center py-16 space-y-4">
                        ...
                        <p className="text-gray-500 text-center">Your completed activities will appear here.</p>
                      </div>
                    )}
                              ...
                              <CardContent className="p-4">
                                <div className="flex items-start space-x-4">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    {userTeam.division && <p className="text-sm text-gray-600 mb-2">{userTeam.division}</p>}
                                  ...
                                  teamMessages.map((message: any) => (
                                    <div key={message.id} className="flex space-x-3 py-3 border-b border-gray-100 last:border-b-0">
                                      <Avatar className="h-8 w-8">
                                        ...
                                        <AvatarFallback
                                          className={
                                            message.sender?.userType === "admin" ? "bg-red-100 text-red-600" : "bg-blue-100 text-blue-600"
                                          }
                                        >
                      ...
                      <div className="space-y-6">
                        <div className="relative">
                          <div className="flex items-center justify-between mb-4">
                            ...
                            <Button variant="ghost" size="icon" onClick={() => setIsEditingProfile((v) => !v)}>
                        ...
                        <Card>
                          <CardContent className="p-4">
                            ...
                            {isEditingProfile ? (
                              ...
                              <p className="text-gray-600">{editableProfile.bio || "Write something about yourself..."}</p>
                            )}
                            {isEditingProfile && (
                              <p className="text-xs text-gray-400 mt-1">{editableProfile.bio.length}/300 characters</p>
                            )}
                          </CardContent>
                        </Card>
                        ...
                        <Card>
                          <CardContent className="p-4">
                            ...
                            <div className="space-y-4">
                              {[
                                { label: "Team Name", key: "teamName", value: currentChild?.teamName || editableProfile.teamName },
                                { label: "Age", key: "age", value: editableProfile.age },
                                { label: "Height", key: "height", value: editableProfile.height },
                                { label: "Weight", key: "weight", value: editableProfile.weight },
                                { label: "Location", key: "location", value: editableProfile.location },
                                { label: "Position", key: "position", value: currentChild?.position || editableProfile.position },
                                { label: "Jersey Number", key: "jerseyNumber", value: currentChild?.jerseyNumber || editableProfile.jerseyNumber },
                              ].map((item) => (
                                <div key={item.key} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                                  ...
                                  {isEditingProfile ? (
                                    ...
                                    <span className="text-gray-600">{(item.value as string) || "—"}</span>
                                  )}
                                </div>
                              ))}
                            </div>
                        ...
                        <Card className="border-0 shadow-sm">
                          <CardContent className="p-4">
                            <div className="flex items-center justify-between mb-3">
                              <h3 className="text-lg font-semibold text-gray-900">Trophies &amp; Badges</h3>
                              <Button
                                variant="ghost"
                                size="sm"
                                className="text-sm text-gray-600 hover:text-gray-800"
                            ...
                            <div className="mt-4">
                              ...
                              <div className="flex flex-wrap gap-2">
                                ...
                                {!awardsLoading &&
                                  (
                                    [
                                      ...(awardsSummary?.recentTrophies ?? []).slice(0, 3).map((a: any) => ({ ...a, _kind: "trophy" })),
                                      ...(awardsSummary?.recentBadges ?? []).slice(0, 3).map((a: any) => ({ ...a, _kind: "badge" })),
                                    ] as any[]
                                  )
                                    .slice(0, 6)
                                    .map((item: any) => (
                                      ...
                                      >
                                        {item.iconUrl ? (
                                          ...
                                          <img src={item.iconUrl} alt={item.name} className="w-7 h-7 rounded-full object-cover" />
                                        ) : item._kind === "trophy" ? (
                                          <Trophy className="w-5 h-5 text-yellow-700" />
                                        ) : (
                                          <Award className="w-5 h-5 text-blue-700" />
                                ...
                                {!awardsLoading &&
                                  !((awardsSummary?.recentTrophies?.length ?? 0) || (awardsSummary?.recentBadges?.length ?? 0)) && (
                                    <div className="text-sm text-gray-500">No recent awards yet.</div>
                                  )}
                              </div>
                            </div>
                          </CardContent>
                          ...
                          <div className="flex justify-end space-x-3">
                            ...
                            <Button
                              onClick={() => {
                                ...
                                // optionally emit profile_updated to backend
