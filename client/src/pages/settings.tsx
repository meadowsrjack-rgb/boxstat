...
function useTab(): [TabKey, (t: TabKey) => void] {
  ...
  const current = useMemo<TabKey>(() => {
    try {
      ...
      return ([
        "profile","privacy","notifications","security","connections","billing","devices","legal","danger"
      ] as TabKey[]).includes(t) ? t : "profile";
    } catch {
      return "profile";
    }
  }, [loc]);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={editableProfile.height} onValueChange={(v) => setEditableProfile(p => ({ ...p, height: v }))}>
                ...
                <SelectContent>
                  {HEIGHT_OPTIONS.map(h => <SelectItem key={h} value={h}>{h}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div>
            ...
            <div>
              ...
              <Select value={editableProfile.jerseyNumber} onValueChange={(v) => setEditableProfile(p => ({ ...p, jerseyNumber: v }))}>
                ...
                <SelectContent>
                  {JERSEY_OPTIONS.map(n => <SelectItem key={n} value={n}>#{n}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                ...
                <Input placeholder="@handle" value={editableProfile.instagram} onChange={(e) => setEditableProfile(p => ({ ...p, instagram: e.target.value }))} />
              </div>
              <div>
        ...
        <CardContent className="p-6 space-y-4">
          {[
            ...
            <div key={row.k} className="flex items-center justify-between rounded-lg border p-3">
              ...
              <Switch
                checked={(state as any)[row.k]}
                onCheckedChange={(v) => setState((s) => ({ ...s, [row.k]: v }))}
              />
            </div>
          ))}
...
function ConnectionsSection() {
  const { toast } = useToast();
  // In a real app, fetch current connection status
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          ...
          <Separator />
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="text-sm text-gray-600">Next charge: <span className="font-medium">$29 on the 15th</span></div>
...
function DevicesSection() {
  // Example: sessions fetched from API
          ...
          {sessions.map((s) => (
            <div key={s.id} className="flex items-center justify-between border rounded-lg p-3">
              <div className="text-sm">
                ...
                <div className="text-xs text-gray-500">{s.location || "—"} • {s.lastActive ? new Date(s.lastActive).toLocaleString() : "Now"}</div>
              </div>
...
function ToggleRow({ label, checked, onChange, defaultChecked }: { label: string; checked?: boolean; onChange?: (v: boolean) => void; defaultChecked?: boolean; }) {
  ...
  useEffect(() => { if (checked !== undefined) setLocal(checked); }, [checked]);
  return (
    <div className="flex items-center justify-between rounded-lg border p-3">
      <div className="text-sm">{label}</div>
      <Switch checked={local} onCheckedChange={(v) => { setLocal(v); onChange?.(v); }} />
    </div>
  );
}
