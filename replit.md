...
- **Coach Dashboard**: Multi-team support with team selection interface. Coaches can view all assigned teams (both head and assistant positions), access full team rosters with app users and Notion-synced players, manage player assignments, evaluate players, award badges/trophies, and access team chat for each assigned team. TeamChat component provides real-time messaging via WebSocket. **Profile-Based Messaging**: Team chat now correctly differentiates between profiles within one account. Messages sent from player dashboard show the player's name (not parent account), messages from coach dashboard show coach's name. Backend validates profile ownership to prevent spoofing, and message alignment (own vs other) works correctly for all profile types. Player removal uses organization-based security checks for safe idempotent roster management. Award system fully integrated with endpoints at `/api/coach/award` for awarding and `/api/users/:userId/{awards|badges|trophies}` for display.
...
- **Admin Panel**: Comprehensive CRUD operations for users, teams, events, awards, divisions, and messages, with robust table views, search functionality, and a calendar view for event management. Detailed user view with sidebar navigation and categorized sections: Team Info, Billing, Performance, Skills & Awards, Admin Notes, and System Meta. Skills evaluations are accessible through the user details window in the Users tab. Awards display correctly across all tables and views through integrated award endpoints.
- **Notifications System**: Comprehensive multi-channel messaging with advanced recipient targeting (everyone, specific users, roles, teams, divisions). **Multi-Type Selection**: Notifications support multiple simultaneous types via checkboxes (announcement, notification, message), allowing combinations like "announcement + notification" to appear in both banner and bell dropdown. Supports in-app, email, and push notification delivery channels. Recipients resolved at send time and stored in notification_recipients join table for auditability. Admin dashboard "Notifications" tab (formerly "Messages") provides creation, listing, and deletion. Multi-channel delivery status tracked per-user in deliveryStatus JSONB field. **Schema**: notifications.types is PostgreSQL text array with validation requiring at least one type.
- **Push Notifications**: Complete web push notification system with service worker, VAPID authentication, and per-recipient delivery tracking. **iOS Support**: Includes iOS PWA detection utilities and install prompts (iOS Safari requires PWA installation before push notifications work). **Setup Required**: Generate VAPID keys by running `node scripts/generate-vapid-keys.js` and add to Replit Secrets: `VAPID_PUBLIC_KEY` and `VAPID_PRIVATE_KEY`. System shows warnings on startup if keys are missing. **Security**: VAPID private keys must never be committed to source code; always use environment variables.
