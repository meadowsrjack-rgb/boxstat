...
-   **Event & Scheduling**: In-app CRUD for events with color-coded UI, player RSVP, and GPS-based check-in (200m geofencing) using OpenStreetMap. Events support configurable player self-RSVP toggles and handle timezones with DST. **Batch Recurring Events**: Recurring event creation uses `POST /api/events/batch` to insert all occurrences in a single request (chunked DB inserts of 100), replacing the previous sequential per-event API calls. Event windows are also created in bulk. Max 1000 events per batch. **Google Calendar Export**: Admins can export upcoming BoxStat events to Google Calendar. The CalendarPlus button opens a dialog showing upcoming events with checkboxes. Selected events can be added directly to Google Calendar (opens Google Calendar event creation page) or downloaded as an .ics file for import into any calendar app. Component: `client/src/components/GoogleCalendarImportDialog.tsx`.
