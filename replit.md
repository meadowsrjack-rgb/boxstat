...
- **Awards System**: Complete 99-trophy/badge system with automatic and manual triggering. Awards are granted via `/api/coach/award` endpoint and displayed through `/api/users/:userId/awards`, `/api/users/:userId/badges`, and `/api/users/:userId/trophies` endpoints. System calls `evaluateAwardsForUser()` after each award to update cached user profiles, ensuring awards appear immediately on player cards, trophies/badges pages, unified account page, and admin dashboard tables.
