...
- **Admin Panel**: Comprehensive CRUD operations for users, teams, events, awards, divisions, and notifications, with robust table views, search functionality, and a calendar view for event management. Detailed user view with sidebar navigation and categorized sections: Team Info, Billing, Performance, Skills & Awards, Admin Notes, and System Meta. Skills evaluations are accessible through the user details window in the Users tab. Awards display correctly across all tables and views through integrated award endpoints.
