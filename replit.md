...
- **Messages System**: Comprehensive multi-channel messaging (renamed from "Notifications") with advanced recipient targeting (everyone, specific users, roles, teams, divisions). Supports in-app, email, push notifications, and SMS delivery channels. Recipients resolved at send time and stored in notification_recipients join table for auditability. Admin-only endpoints at `/api/admin/notifications` handle creation, listing, and deletion. Multi-channel delivery status tracked per-user in deliveryStatus JSONB field. **SMS Integration**: Infrastructure complete and ready for Twilio credentials (TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER); shows "pending_credentials" status until configured.
