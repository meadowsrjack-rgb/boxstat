...
-   **Payment & Awards**: Stripe integration for secure payments supporting various billing models, and a comprehensive 100-trophy/badge achievement system. Payment labels use clear Subscription/Prepaid/One-time purchase terminology: options with `billingCycle !== "One-Time"` show as "Subscription · Billed [interval] · Cancel anytime" (purple), options with `durationDays` and `billingCycle === "One-Time"` show as "Prepaid · X-day access", and simple one-time items show as "One-time purchase". Programs with `type="Subscription"` but `billingCycle="One-time"` are handled gracefully (not shown as top-level subscription). Payment history badges: amber "Subscription" with refresh icon, blue "Prepaid" with credit card icon.
...
-   **Notifications**: Multi-channel (in-app, email, push) notification system with advanced recipient targeting and web push support. Features scheduled and recurring messaging with campaign management. Includes automatic scheduled notifications: event reminders (24h, 2h, 30min before), check-in availability alerts, and RSVP window closing warnings. **Notification bell behavior**: Opening the notification bell popover does NOT auto-mark notifications as read — each must be clicked individually, or user can explicitly click "Mark all read" in the popover header. **Admin action banners**: Admin dashboard Overview tab shows red-themed banners for items needing attention: `CrmMessageBanner` (unread CRM messages, "View Messages" navigates to CRM messages sub-tab), `StorePurchaseBanner` (store orders needing dispatch, powered by `GET /api/admin/pending-orders`), and `EnrollmentAssignmentBanner` (new enrollments needing team/skill assignment, powered by `GET /api/admin/pending-assignments`). **Store purchase + enrollment notifications**: When a store item is purchased, admins get push + in-app notification ("New Store Order — dispatch required"). When a player enrolls in a program, admins get notified to assign team/skill level. `notifyAllAdmins` supports optional `organizationId` scoping for tenant isolation. **Attendance-based notifications**: Daily cron job (`0 8 * * *`) analyzes player attendance patterns and sends streak/miss notifications to all four roles (player, parent, coach, admin). Real-time attendance notifications also fire on check-in milestones (3/5/10/15/20 attend streaks, welcome-back). Key files: `server/services/attendanceTracker.ts` (analysis), `pushNotificationHelper.ts` (notification functions), `notificationScheduler.ts` (daily job). Deduplication via title+timestamp check prevents re-sending within 7-day (streaks) or 30-day (perfect month) windows. **Abandoned cart notifications**: Tracks when parents start checkout but don't complete it. `abandoned_carts` table records each checkout session creation; on Stripe webhook completion, the cart is marked complete. A cron job (`*/15 * * * *`) sends timed reminders at 1 hour, 24 hours, and 3 days. Frontend `AbandonedCartBanner` component shows on the parent Home and Payments tabs with dismiss capability. Key files: `client/src/components/AbandonedCartBanner.tsx`, `pushNotificationHelper.ts` (cart reminder functions).
...
-   **Android App**: Capacitor Android platform configured with BoxStat branding (red accent #d82428), GPS/location permissions, push notification support, and `boxstat://` deep links. Loads from the live server (`boxstat.app`). See `GOOGLE_PLAY_GUIDE.md` for build and publish instructions.
