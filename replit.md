...
- **Parental Device Lock**: Parents can lock a device to a specific child's dashboard, preventing access to the account page and other player profiles. Lock is client-side using localStorage with key `deviceLockedToPlayer`. When locked, back navigation is blocked and replaced with a lock icon indicator. Player settings includes an unlock button to escape the lock and return to the account page. All components synchronize lock state via custom `deviceLockChanged` event. Player cards show lock status with color-coded icons (red when locked, gray when unlocked) and visual "Device locked to this player" messages.
