...
-   **Admin Team Assignment with Auto-Enrollment**: Facilitates player assignment to teams with automatic enrollment into associated programs. When assigning unenrolled players, admins can optionally set an enrollment expiry date (grace period). With a date: player gets full access until then and is prompted to pay before expiry. Without a date: player appears on roster but must enroll and pay through payments for full team access. Migration wizard works the same — only creates enrollment when `subscriptionEndDate` is provided in CSV.
-   **Event & Scheduling**: In-app CRUD for events with color-coded UI, player RSVP, and GPS-based check-in (200m geofencing) using OpenStreetMap. Events support configurable player self-RSVP toggles and handle timezones with DST. **Batch Recurring Events**: Recurring event creation uses `POST /api/events/batch` to insert all occurrences in a single request (chunked DB inserts of 100), replacing the previous sequential per-event API calls. Event windows are also created in bulk. Max 1000 events per batch. **iCal Import**: Admins can import events from Google Calendar (or any calendar app) by uploading an .ics file. The frontend parses the iCal format (handling timezones, all-day events, recurrence rules) and shows a preview with checkboxes. Selected events are batch-imported via `POST /api/ical/import`. Imported events default to "Everyone" visibility and event type "other". Component: `client/src/components/GoogleCalendarImportDialog.tsx`, service: `server/services/googleCalendar.ts`.
...
-   **XLSX/CSV Bulk User Import**: Admin bulk-upload supports both CSV and XLSX files. XLSX files are parsed by sheet (Parents, Players, Coaches, Admins) using SheetJS. Teams and Programs have a `code` field (short import code e.g. "THU12", "SKA") visible in list views and create/edit dialogs, matched during import before falling back to name matching. Imported users get a welcome/claim-account email via Resend with App Store link and magic link. Bulk import uses the `/api/users/bulk-import` batch endpoint. Parents are processed first so children can link by parentEmail.
...
-   **Program Visibility (Lead vs Member)**: Programs have a `visibility` field (`'public'` or `'members_only'`). Public programs are visible to all users (leads). Members-only programs are hidden from users who don't have any active enrollment. Admins always see all programs. Enforced on both `GET /api/programs` (list) and `GET /api/programs/:id` (detail). Uses the same `accountHolderId` resolution pattern as enrollments to properly handle parent/child accounts.
