...
- **User Management**: Single Parent account with linked child profiles; Dual Mode System (Parent/Player) secured by PIN. Soft delete system for user accounts - deleted users have deletedAt timestamp set, are blocked from authentication, excluded from all user listings, and have their sessions immediately invalidated. Children are automatically soft deleted when parent is deleted. Payment/audit data is preserved.
